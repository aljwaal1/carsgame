...
class Sfx {
  ...
  static void lowFuel(int tick) {
    if (tick - _fuelAlarmTick > 28) {
      _fuelAlarmTick = tick;
      play('sounds/shared/game_over.wav', .24);
    }
  }
}
          ...
          child: SafeArea(
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                ...
                const Text('ألعاب زمان V5.2', textAlign: TextAlign.center, style: TextStyle(fontSize: 35, fontWeight: FontWeight.w900)),
                const SizedBox(height: 8),
                const Text('إنذار وقود للطائرة + رجوع شكل السيارات الأجمل', textAlign: TextAlign.center, style: TextStyle(color: Colors.white70)),
                const SizedBox(height: 28),
                GameTile(title: 'طريق التحمل', icon: '🏎️', text: 'رجعنا الطريق والشكل الأقرب للنسخة الأجمل.', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const Directionality(textDirection: TextDirection.rtl, child: RoadGame())))),
                const SizedBox(height: 16),
                GameTile(title: 'طائرة الوقود', icon: '✈️', text: 'إنذار عند قرب الوقود من الانتهاء + GAS على العبوة.', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const Directionality(textDirection: TextDirection.rtl, child: PlaneGame())))),
                const Spacer(),
...
class HintBox extends StatelessWidget {
  ...
  final String text;
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 7), decoration: BoxDecoration(color: Colors.black.withOpacity(.30), borderRadius: BorderRadius.circular(13), border: Border.all(color: Colors.white12)), child: Text(text, style: const TextStyle(fontSize: 10, color: Colors.white70)));
}
...
class OverlayCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [Icon(danger ? Icons.warning_amber_rounded : Icons.play_circle_fill, size: 52, color: danger ? Colors.redAccent : Colors.cyanAccent), const SizedBox(height: 8), Text(title, textAlign: TextAlign.center, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900)), const SizedBox(height: 8), Text(subtitle, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white70)), const SizedBox(height: 18), RetroButton(text: button, icon: Icons.play_arrow, onTap: onTap)]),
      );
}
...
class EndCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [SizedBox(height: 115, child: CustomPaint(painter: EndPainter(plane: plane), child: const SizedBox.expand())), const SizedBox(height: 8), Text(title, style: const TextStyle(fontSize: 32, fontWeight: FontWeight.w900, color: Colors.redAccent)), const SizedBox(height: 6), Text('النقاط: $score   |   $extra', textAlign: TextAlign.center, style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.bold)), const SizedBox(height: 18), RetroButton(text: 'إعادة اللعب', icon: Icons.replay, onTap: onTap)]),
      );
}
...
class _PlaneGameState extends State<PlaneGame> {
  ...
  void step() {
    ...
    if (fuel < 22 && tick > 45) Sfx.lowFuel(tick);
    if (tick % 10 == 0) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('طائرة الوقود V5.2')),
...
class PlanePainter extends CustomPainter {
  ...
  void drawObject(Canvas c, Size s, PlaneObj o) {
    ...
    if (o.kind == 0) {
      ...
      final gasText = TextPainter(text: const TextSpan(text: 'GAS', style: TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: .4)), textDirection: TextDirection.ltr, textAlign: TextAlign.center)..layout();
...
class _RoadGameState extends State<RoadGame> {
  ...
  Widget build(BuildContext context) {
    final overlay = !running || stopped || gameOver;
    return Scaffold(
      appBar: AppBar(title: const Text('طريق التحمل V5.2')),
      body: SafeArea(child: Column(children: [
...
class RoadPainter extends CustomPainter {
  ...
  void drawRoadDetails(Canvas c, Size s) {
    for (var i = 0; i < 20; i++) {
      ...
      if (i % 2 == 0) {
        c.drawRRect(RRect.fromRectAndRadius(Rect.fromCenter(center: Offset(s.width * .5, y), width: 2 + d * 8, height: 12 + d * 48), const Radius.circular(4)), Paint()..color = Colors.white.withOpacity(v * (weather == 'ليل' ? .36 : weather == 'ضباب' ? .28 : .70)));
      }
    }
  }
  ...
  void drawSuperCar(Canvas c, Offset o, double w, Color body, bool opponent, double opacity) {
    ...
    c.drawRRect(RRect.fromRectAndRadius(Rect.fromCenter(center: o.translate(0, -h * .01), width: w * .34, height: h * .30), Radius.circular(w * .10)), Paint()..color = const Color(0xffdbeafe).withOpacity(.92 * opacity));
  ...
  void drawCrashFlash(Canvas c, Size s) {
    ...
    for (var i = 0; i < 12; i++) {
      final a = i * math.pi / 6;
      c.drawLine(center, Offset(center.dx + math.cos(a) * (36 + 18 * op), center.dy + math.sin(a) * (25 + 14 * op)), spark);
    }
  }
